...
For the comparative modeling stage, I target a stratified 40,000-observation analysis sample drawn after the initial train/test split. This enlarged sample strengthens the main comparison among random forest, XGBoost, and logistic regression without changing the cleaned feature logic. The only smaller sample used in the report itself is the SHAP interpretation subset, which is used only for computational convenience after the preferred model has already been fit.
...
The metric rule in the rest of the report has two steps. First, I choose the underlying classifier by ROC AUC because that metric evaluates ranking performance without fixing a cutoff. Second, after the underlying classifier is chosen, I compare operating thresholds. At that stage, balanced accuracy is the main summary because it keeps the minority and majority classes in view at the same time. But I do not treat it mechanically by itself. For this application, fatal recall still matters more than precision or raw accuracy alone, because missing a truly fatal incident is more costly than flagging some non-fatal incidents as higher risk. At the same time, I do not want false positives to grow without bound, so non-fatal specificity also matters. In other words, ROC AUC chooses the underlying classifier, while balanced accuracy helps compare thresholds, and recall plus specificity determine whether a threshold is practically acceptable for the screening-style objective. For the final operating rule, I therefore compare balanced-accuracy-based threshold moving with a more conservative specificity-constrained threshold check.

## 6. Baseline Model Comparison
The baseline comparison uses the saved held-out results from `three_model_default_results.csv`, which apply the same cleaned feature set and default threshold of 0.50 on the 40,000-observation modeling sample.
...
```{r}
#| label: threshold-constrained-logic
#| echo: true
#| eval: false
...
The overall conclusion is cautious rather than dramatic. Structured incident-level features cannot support a strong or broadly deployable classifier here, but they do support limited screening signal for separating fatal from non-fatal incidents. Much of that usable signal appears to be contextual, especially from location and timing variables, rather than deeply incident-specific. Under that framing, random forest is the strongest underlying classifier in the direct comparison by ROC AUC, and the most defensible final operating rule is the more conservative specificity-constrained threshold-moving version of that model. This improves fatal-case detection relative to the default cutoff while keeping non-fatal specificity near the intended bound for the paper's screening-style objective. If the main priority is to catch more fatal incidents and accept more false positives, XGBoost with oversampling is the more aggressive alternative.
