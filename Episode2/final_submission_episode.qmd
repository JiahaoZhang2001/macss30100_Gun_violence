...
default_results_path <- "three_model_default_results.csv"
imbalance_results_path <- "model_results_imbalance_strategies.csv"
threshold_results_path <- "threshold_results_balanced_accuracy.csv"
...
Repository / shared materials link: [final public link to be inserted]

## 2. Data and Outcome Definition
The original records come from Gun Violence Archive incident listings that had already been assembled for the course project into a local structured file. The analysis in this report begins from [`stage3.csv`](/Users/jiahaozhang/Desktop/macs%2030100/project/Episode2/Data/stage3.csv), which is the project’s cleaned stage-three working dataset rather than a live scrape. It contains incident-level observations, original URLs, and a larger set of structured fields created earlier in the project workflow. In other words, `stage3.csv` is the final local analysis file derived from the original incident-source records, and every table and figure below is produced from that file or from saved result tables generated from it.
...
For presentation in the PDF report, I keep this static density figure because it is easier to read on the page. I also generated an interactive leaflet version as a supplementary visualization of the same spatial pattern. The interactive version is useful for browsing local concentration more directly, but the static density figure is better suited to the final written report.

The next screenshot shows the interactive supplement in a fixed PDF-friendly form. I include it only as a companion visualization, not as a main analytical figure, because the static density plot above is easier to read in print while the interactive version is better for browsing local clusters and pop-up details. The standalone supplement is provided separately as `interactive_incident_map.html`.

```{r}
#| label: leaflet-map-screenshot
#| out-width: "90%"
#| fig-align: center
knitr::include_graphics("images/interactive_incident_map_preview.png")
```

The code below shows the core construction of that leaflet supplement. I keep only the main mapping steps here, because the final submission is a PDF report and the full interactive file is provided separately.

```{r}
#| label: leaflet-map-code
#| echo: true
#| eval: false
library(leaflet)
...
map_df <- stage3 |>
  ...
  mutate(
    fatal = if_else(n_killed > 0, "fatal", "non_fatal")
  )
...
leaflet(map_df) |>
  ...
  addCircleMarkers(
    ...
    lat = ~latitude,
    radius = 2,
    color = ~if_else(fatal == "fatal", "#C0392B", "#7F8C8D"),
    fillOpacity = 0.5,
    stroke = FALSE
  ) |>
  setView(
    lng = mean(map_df$longitude, na.rm = TRUE),
    lat = mean(map_df$latitude, na.rm = TRUE),
    zoom = 4
  ) |>
  addLegend(
    position = "bottomright",
    colors = c("#7F8C8D", "#C0392B"),
    labels = c("non_fatal", "fatal"),
...
The metric rule in the rest of the report is simple. I select the underlying classifier by ROC AUC because it evaluates ranking performance without fixing a cutoff. I then select the final operating threshold by balanced accuracy, while using fatal recall and non-fatal specificity to interpret the practical trade-off. For this application, I still place somewhat more weight on recall for the fatal class than on precision or raw accuracy alone, because missing a truly fatal incident is more costly than flagging some non-fatal incidents as higher risk. At the same time, I do not want false positives to grow without bound. That is why recall informs the interpretation, but does not replace ROC AUC or balanced accuracy as the main selection rules.

## 6. Baseline Model Comparison
The baseline comparison uses the saved held-out results from [`three_model_default_results.csv`](/Users/jiahaozhang/Desktop/macs%2030100/project/Episode2/three_model_default_results.csv), which apply the same cleaned feature set and default threshold of 0.50 on the 40,000-observation modeling sample.
...
#| out-width: "90%"
#| fig-align: center
knitr::include_graphics("diagnostics/plots/probability_distributions.png")
```
...
The main finding is fairly modest, but still useful. Structured incident-level features do contain predictive signal for fatal versus non-fatal outcomes, but the signal is limited and easy to overstate if evaluation focuses only on accuracy. This is consistent with the broader literature: administrative and surveillance-style records can support predictive screening, but their usefulness depends heavily on data quality, missingness, and careful modeling choices [@goin2018; @heller2022; @swedo2023].
